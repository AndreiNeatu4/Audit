...
async function scanProfile(browser, url, profile, agg, extraAgg, capture) {
  ...
  try {
    ...
    // Grab outbound links (first profile only) — used by the site crawler to
    // discover further pages. Harmless/unused for a plain single-page scan.
    if (capture && !capture.linksDone) {
      try {
        capture.links = await page.$$eval('a[href]', (as) => as.map((a) => a.href));
      } catch {
        capture.links = [];
      } finally {
        capture.linksDone = true;
      }
    }
 ...
 * reusing an already-launched Playwright browser instance. Used directly by
 * the site crawler (one browser shared across many pages); analyzeUrl() below
 * wraps this for the single-page-scan API, launching/closing its own browser.
 * @param {import('playwright').Browser} browser
 * @param {string} url
 * @param {{ profiles?: string[] }} opts  which profile ids to run (defaults to all)
 */
async function analyzePageWithBrowser(browser, url, opts = {}) {
  ...
  const capture = { done: false, screenshotBase64: null, html: null, linksDone: false, links: [] };
  const profileErrors = [];
...
/**
 * Analyze a single URL, launching and closing its own browser. Thin wrapper
 * around analyzePageWithBrowser() for the single-page-scan API.
 * @param {string} url
...
async function analyzeUrl(url, opts = {}) {
  ...
  try {
    return await analyzePageWithBrowser(browser, url, opts);
  } finally {
    await browser.close();
  }
}

module.exports = { analyzeUrl, analyzePageWithBrowser, normalizeUrl, PROFILES };
